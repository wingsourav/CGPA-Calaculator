...
const addSubject = (card, semesterNumber = [...semesters.children].indexOf(card) + 1) => {
  ...
  card.querySelector('.subject-list').append(row);
};
...
semesters.addEventListener('click', event => {
  ...
  if (event.target.matches('.remove-subject')) event.target.closest('.subject-row').remove();
...
document.querySelector('#calculate').onclick = () => {
  ...
  if (!valid || !totalCredits) { error.textContent = 'Please enter a valid credit and grade for every subject you add.'; error.style.display = 'block'; document.querySelector('#result').style.display = 'none'; return; }
...
document.querySelector('#calculateGrade').onclick = () => {
  ...
  if (!valid || !hasEntry) { error.textContent = 'Enter a valid credit and SGPA (0–10) for each completed semester.'; error.style.display = 'block'; return; }
  error.style.display = 'none';
